...
app.post('/api/items', (req, res) => {
  const { text, category, status, priority, notes } = req.body;
  const result = db.prepare('INSERT INTO items (text, category, status, priority, notes) VALUES (?,?,?,?,?)').run(text, category || 'Inbox', status || 'Todo', priority || 'Normal', notes || null);
  res.json(db.prepare('SELECT * FROM items WHERE id=?').get(result.lastInsertRowid));
});

app.put('/api/items/:id', (req, res) => {
  const { text, category, status, priority, notes } = req.body;
  db.prepare('UPDATE items SET text=?, category=?, status=?, priority=?, notes=?, updated_at=CURRENT_TIMESTAMP WHERE id=?').run(text, category, status, priority, notes, req.params.id);
